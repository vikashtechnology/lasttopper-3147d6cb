...
function Notifications() {
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`notif-${meId}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "notifications", filter: `user_id=eq.${meId}` },
        () => { qc.invalidateQueries({ queryKey: ["notifications"] }); qc.invalidateQueries({ queryKey: ["notif-unread"] }); })
      .subscribe();
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="mx-auto max-w-3xl px-4 py-6 space-y-2">
        {list.data?.length ? list.data.map((n) => (
          <div
            key={n.id}
            onClick={() => { if (n.link) navigate({ to: n.link }); }}
