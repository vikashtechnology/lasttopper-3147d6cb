...
function PostDetail() {
  ...
  const post = useQuery({ queryKey: ["forum-post", postId], queryFn: () => getForumPost({ data: { post_id: postId } }) });
  ...
  const vote = useMutation({
    mutationFn: (v: { target_type: "post" | "reply"; target_id: string; value: 1 | -1 | 0 }) => voteOnTarget({ data: v }),
  ...
  return (
    <section className="mx-auto max-w-3xl px-4 py-6">
      <article className="rounded-2xl border border-border bg-card p-5">
        ...
        {p.author?.id && (
          <Link to="/profile/$userId" params={{ userId: p.author.id }} className="mt-1 block text-xs text-muted-foreground hover:underline">
            by {p.author.full_name ?? "Anon"}
          </Link>
        )}
        <div className="mt-3 whitespace-pre-wrap text-sm">{p.body}</div>
        <div className="mt-4 flex items-center gap-2">
          <Button size="sm" variant={post.data.my_vote === 1 ? "default" : "outline"}
            onClick={() => vote.mutate({ target_type: "post", target_id: p.id, value: post.data!.my_vote === 1 ? 0 : 1 })}>
            <ArrowUp className="h-4 w-4" /><span className="ml-1">{p.upvote_count}</span>
          </Button>
          <Button size="sm" variant="ghost" onClick={() => flag.mutate({ target_type: "forum_post", target_id: p.id })}>
      ...
      <div className="mt-6 space-y-3">
        <h2 className="flex items-center gap-2 text-sm font-semibold"><MessageCircle className="h-4 w-4" />{p.reply_count} replies</h2>
        {post.data.replies.map((r) => (
          <div key={r.id} className="rounded-xl border border-border bg-card p-4">
            ...
            <div className="mt-2 flex items-center gap-2">
              <Button size="sm" variant="ghost" onClick={() => vote.mutate({ target_type: "reply", target_id: r.id, value: 1 })}>
                <ArrowUp className="h-4 w-4" /><span className="ml-1">{r.upvote_count}</span>
              </Button>
              <Button size="sm" variant="ghost" onClick={() => flag.mutate({ target_type: "forum_reply", target_id: r.id })}>
      ...
      <div className="mt-6">
        <Textarea placeholder="Write a reply…" value={body} onChange={(e) => setBody(e.target.value)} rows={4} maxLength={4000} />
