...
import { listDoubts, createDoubt, createDoubtImageUploadUrl, getDoubtImageUrl } from "@/lib/community.functions";
  ...
  return (
    <section className="mx-auto max-w-4xl px-4 py-6">
      <div className="mb-4 flex items-center gap-2">
        <Input placeholder="Search doubts…" value={q} onChange={(e) => setQ(e.target.value)} className="flex-1" />
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button size="sm"><Plus className="mr-1 h-4 w-4" />Ask</Button>
          </DialogTrigger>
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <Textarea placeholder="Describe your doubt…" value={body} onChange={(e) => setBody(e.target.value)} maxLength={4000} rows={6} />
              <label className="flex cursor-pointer items-center gap-2 rounded-md border border-dashed border-border p-3 text-sm text-muted-foreground hover:bg-muted/50">
                ...
                <span>{imagePath ? "Image attached ✓" : uploading ? "Uploading…" : "Attach image (optional)"}</span>
                <input type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) void handleFile(f); }} />
              </label>
      ...
      <div className="space-y-2">
        {doubts.data?.map((d) => (
          <Link key={d.id} to="/community/doubt/$doubtId" params={{ doubtId: d.id }}
            ...
            <div className="flex items-start gap-3">
              <div className="flex-1">
                ...
                <div className="mt-2 flex items-center gap-3 text-xs text-muted-foreground">
                  ...
                  <span className="inline-flex items-center gap-1"><ArrowUp className="h-3 w-3" />{d.upvote_count}</span>
                  <span className="inline-flex items-center gap-1"><MessageCircle className="h-3 w-3" />{d.reply_count}</span>
                </div>
              </div>
              {d.image_url && <img src={d.image_url} alt="" className="h-16 w-16 rounded-lg object-cover" />}
            </div>
          </Link>
        ))}
