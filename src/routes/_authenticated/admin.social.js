...
import { adminListSocialLinks, adminSaveSocialLink, adminDeleteSocialLink, } from "@/lib/social.functions";
...
function AdminSocial() {
    ...
    const save = useMutation({
        mutationFn: (row) => adminSaveSocialLink({ data: row }),
        onSuccess: () => {
            toast.success("Saved");
    ...
    const remove = useMutation({
        mutationFn: (id) => adminDeleteSocialLink({ data: { id } }),
        onSuccess: () => {
    ...
    return (<section className="mx-auto max-w-3xl space-y-8 px-4 py-6">
      ...
      <div className="mt-4 space-y-3">
        ...
        {rows.map((row) => (<LinkRow key={row.id} row={row} onSave={(patch) => save.mutate({ ...row, ...patch })} onDelete={() => remove.mutate(row.id)}/>))}
      </div>
...
function LinkRow({ row, onSave, onDelete, }) {
    ...
    return (<div className="rounded-2xl border border-border bg-card p-3">
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          <Switch checked={row.enabled} onCheckedChange={(v) => onSave({ url: url.trim(), enabled: v })} aria-label={`Show ${row.label}`}/>
