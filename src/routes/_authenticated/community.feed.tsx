    ...
    <section className="mx-auto max-w-3xl px-4 py-6 space-y-6">
      <div>
        <h2 className="mb-3 flex items-center gap-2 text-sm font-semibold text-muted-foreground">
          <TrendingUp className="h-4 w-4" />Trending doubts
        </h2>
        <div className="space-y-2">
          {feed.data?.trending_doubts.map((d) => (
            <Link key={d.id} to="/community/doubt/$doubtId" params={{ doubtId: d.id }}
              ...
              <div className="text-xs text-muted-foreground">↑ {d.upvote_count} · 💬 {d.reply_count}</div>
            </Link>
          ))}
      ...
      <div>
        <h2 className="mb-3 flex items-center gap-2 text-sm font-semibold text-muted-foreground">
          <Activity className="h-4 w-4" />Following
        </h2>
        <div className="space-y-2">
          {feed.data?.events.map((e) => (
            <div key={e.id} className="rounded-xl border border-border bg-card p-3 text-sm">
              <div className="flex items-center gap-2">
                {e.kind === "doubt_created" ? <HelpCircle className="h-3.5 w-3.5 text-primary" /> : <MessageSquare className="h-3.5 w-3.5 text-primary" />}
              ...
              {typeof e.payload === "object" && e.payload && "title" in e.payload && (
                <div className="mt-1 text-muted-foreground">{String((e.payload as Record<string, unknown>).title)}</div>
              )}
            </div>
          ))}
