...
import { getStudyGroup, listGroupMessages, sendGroupMessage, joinStudyGroup, leaveStudyGroup } from "@/lib/community.functions";
...
function GroupChat() {
  ...
  const info = useQuery({ queryKey: ["group", groupId], queryFn: () => getStudyGroup({ data: { group_id: groupId } }) });
  const msgs = useQuery({
  ...
  return (
    <section className="mx-auto flex max-w-4xl flex-col px-4 py-4" style={{ height: "calc(100vh - 120px)" }}>
      <div className="mb-3 rounded-2xl border border-border bg-card p-4">
        <div className="flex items-center gap-3">
          ...
          {info.data.is_member ? (
            <Button size="sm" variant="outline" onClick={() => leave.mutate()} disabled={leave.isPending}>
      ...
      {info.data.is_member ? (
        <>
          <div ref={scrollRef} className="flex-1 space-y-2 overflow-y-auto rounded-2xl border border-border bg-card p-4">
            {msgs.data?.map((m) => (
          ...
          >
            ...
            <Button type="submit" disabled={send.isPending || !text.trim()}><Send className="h-4 w-4" /></Button>
          </form>
        </>
      ) : (
