...
function DoubtDetail() {
  ...
  const accept = useMutation({
    ...
    onSuccess: () => {
      toast.success("Marked as accepted (+10 rep)");
      qc.invalidateQueries({ queryKey: ["doubt", doubtId] });
    },
  });
  const flag = useMutation({
    mutationFn: (id: string) =>
      reportContent({ data: { target_type: "doubt_reply", target_id: id, reason: "Reported" } }),
    ...
    <section className="mx-auto max-w-3xl px-4 py-6">
      ...
      <div className="mt-6 space-y-3">
        ...
        {d.data.replies.map((r) => (
          <div
            key={r.id}
            className={`rounded-xl border p-4 ${r.is_accepted ? "border-green-500/60 bg-green-500/5" : "border-border bg-card"}`}
          >
