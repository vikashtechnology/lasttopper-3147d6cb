...
async function tgReply(chatId: number | string, text: string) {
  const token = (process.env.TELEGRAM_API_KEY_1 ?? process.env.TELEGRAM_API_KEY)?.trim();
  if (!token) return;
      ...
      POST: async ({ request }) => {
        ...
        if (!m) {
          if (/^\/(approve|reject)\b/i.test(text)) {
            await tgReply(
              chatId,
              "Usage: <code>/approve id=123456</code> or <code>/reject id=123456</code>",
            );
          }
        ...
        if (action === "approve") {
          ...
          await tgReply(
            chatId,
            `✅ Approved withdrawal <b>#${shortCode}</b> — ₹${wr.amount} (${wr.method.toUpperCase()}).`,
          );
        } else {
          ...
          await supabaseAdmin.from("wallet_transactions").insert({
            ...
            balance_after: next,
            note: `Withdrawal #${shortCode} rejected — refund`,
            reference_id: wr.id,
          });
          await supabaseAdmin
            .from("withdrawal_requests")
          ...
          await tgReply(
            chatId,
            `🚫 Rejected withdrawal <b>#${shortCode}</b> — ₹${wr.amount} refunded to user's wallet.`,
          );
        }
        return Response.json({ ok: true });
      },
