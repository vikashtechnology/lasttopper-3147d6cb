...
async function callGeminiMega(count: number, batchIdx: number): Promise<MegaQuestion[]> {
  ...
  try {
    ...
    const parsed = JSON.parse(content) as {
      questions?: Array<Omit<MegaQuestion, "id" | "chapter_id">>;
    };
          ...
          if (paidEntries.length < t.min_participants) {
            for (const e of paidEntries) {
              ...
              await supabaseAdmin.from("wallet_transactions").insert({
                ...
                amount: t.entry_fee,
                balance_after: bal,
                note: "Mega Test refund — under-subscribed",
                reference_id: t.id,
              });
              await supabaseAdmin
                .from("mega_test_entries")
          ...
          for (let i = 0; i < ranked.length; i += 1) {
            ...
            if (rank === 1 && paidEntries.length >= 50) {
              ...
              const { data: u2 } = await supabaseAdmin
                ...
                .maybeSingle();
              const base =
                u2?.pro_until && new Date(u2.pro_until) > new Date()
                  ? new Date(u2.pro_until)
                  : new Date();
